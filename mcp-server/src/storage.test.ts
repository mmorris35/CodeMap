...
describe("CodeMapStorage", () => {
  ...
  describe("saveCodeMap", () => {
    ...
    it("should validate CodeMap schema before saving", async () => {
      ...
      await expect(
        storage.saveCodeMap("user1", "project1", invalid),
      ).rejects.toThrow();
    });
    ...
    it("should reject missing required fields", async () => {
      ...
      await expect(
        storage.saveCodeMap("user1", "project1", incomplete),
      ).rejects.toThrow();
    });
    ...
    it("should allow different users to have same project ID", async () => {
      ...
      const stored1 = JSON.parse(
        (await mockKV.get("user:user1:project:my-app"))!,
      );
      const stored2 = JSON.parse(
        (await mockKV.get("user:user2:project:my-app"))!,
      );
  ...
  describe("deleteCodeMap", () => {
    ...
    it("should be safe to call on non-existent project", async () => {
      // Should not throw
      await expect(
        storage.deleteCodeMap("user1", "nonexistent"),
  ...
  describe("saveCache", () => {
    ...
    it("should allow different users to cache same query", async () => {
      ...
      const cached1 = JSON.parse(
        (await mockKV.get("user:user1:cache:query-hash"))!,
      );
      const cached2 = JSON.parse(
        (await mockKV.get("user:user2:cache:query-hash"))!,
      );
  ...
  describe("deleteCache", () => {
    ...
    it("should be safe to call on non-existent cache", async () => {
      await expect(
        storage.deleteCache("user1", "nonexistent"),
      ).resolves.not.toThrow();
    });
